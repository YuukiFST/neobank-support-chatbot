...
- [O braço multi-agente precisa usar tool calling do LLM?](issues/09-o-que-esta-sendo-comparado.md) — sim, obrigatoriamente. Com acurácia de seleção como desfecho primário, um braço que escolhe ferramenta por dicionário não tem seleção para medir. `INTENT_TOOLS` deixa de despachar e passa a filtrar **visibilidade** de ferramenta, que é a variável independente. Confirmado no código: `llm.py:17` usa prefixo `ollama/` (precisa `ollama_chat/` para aceitar `tools`) e `llm.py:67-72` descarta `tool_calls`. Mudar o título para "roteamento determinístico contra roteamento pelo modelo" ficou fora de escopo.
- [Defeitos do braço A que contaminam o experimento](issues/14-defeitos-do-braco-a.md) — corrigir os defeitos 1, 3 e 4 antes de qualquer execução; declarar 2 e 5 como limitação. O defeito 1 (três intents nunca executam a ferramenta que prometem) deixou de ser assimetria entre braços e virou impedimento de medição: 4 dos 15 casos teriam acurácia zero em todas as condições, sem discriminar nenhuma. `expected_tool`, que hoje nenhum código lê, vira o gabarito do oráculo. O artigo relata o estado corrigido e cita o commit congelado.
